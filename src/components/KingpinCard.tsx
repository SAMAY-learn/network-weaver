...
import { useWatchlist, WatchlistPriority } from '@/hooks/useWatchlist';
import { useReviewFlags } from '@/hooks/useReviewFlags';
import { printKingpinReport } from '@/lib/kingpinReportUtils';
...
const KingpinCard = ({ kingpin, rank, delay = 0, onClick, compact = false }: KingpinCardProps) => {
  ...
  const { isFlagged, flagForReview, removeFlag } = useReviewFlags();

  const isWatched = isOnWatchlist(kingpin.id);
  const isFlaggedForReview = isFlagged(kingpin.id);
  ...
  const handleGenerateReport = async (e: React.MouseEvent) => {
    ...
    try {
      await printKingpinReport(kingpin);
    } finally {
  ...
  const handleWatchlistClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (isWatched) {
      removeFromWatchlist.mutate(kingpin.id);
    } else {
      setShowWatchlistDialog(true);
    }
  };

  const handleWatchlistConfirm = (priority: WatchlistPriority, notes?: string) => {
    addToWatchlist.mutate({ suspectId: kingpin.id, priority, notes });
  ...
  const handleFlagConfirm = (reason: string) => {
    flagForReview.mutate({ suspectId: kingpin.id, reason });
  ...
  return (
    <>
      ...
      >
        ...
        {/* Watchlist & Flag Indicators */}
        <div className="absolute top-2 left-2 flex gap-1 z-10">
          {isWatched && (
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="w-6 h-6 rounded-full bg-warning/20 border border-warning/50 flex items-center justify-center"
              title="On Watchlist"
            >
          ...
          {isFlaggedForReview && (
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="w-6 h-6 rounded-full bg-destructive/20 border border-destructive/50 flex items-center justify-center"
          ...
          <div className="flex items-center justify-center gap-2">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>Generate Intelligence Report</TooltipContent>
              </Tooltip>
            </TooltipProvider>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className={`h-7 px-2 ${isWatched 
                      ? 'bg-warning/20 text-warning hover:bg-warning/30' 
                      : 'bg-secondary/80 hover:bg-warning hover:text-warning-foreground'
                    }`}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className={`h-7 px-2 ${isFlaggedForReview 
                      ? 'bg-destructive/20 text-destructive hover:bg-destructive/30' 
                      : 'bg-secondary/80 hover:bg-destructive hover:text-destructive-foreground'
                    }`}
                    onClick={handleFlagClick}
                    disabled={flagForReview.isPending || removeFlag.isPending}
                  >
                    {flagForReview.isPending || removeFlag.isPending ? (
                      <Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" />
                    ) : (
                      <Flag className={`w-3.5 h-3.5 mr-1 ${isFlaggedForReview ? 'fill-current' : ''}`} />
                    )}
                    {isFlaggedForReview ? 'Unflag' : 'Flag'}
                  </Button>
                </TooltipTrigger>
                <TooltipContent>{isFlaggedForReview ? 'Remove Flag' : 'Flag for Review'}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
        </motion.div>
      ...
      <FlagReviewDialog
        ...
        isLoading={flagForReview.isPending}
      />
    </>
  );
};
